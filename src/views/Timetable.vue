...
export default {
  ...
  methods: {
    ...
    /**
     * 处理数据
     * @param { Number } dayIndex 周几对应的下标
     * @param { Number }  courseIndex  第几节课对应的下标
     * @returns { String }   返回对应字符
     */
    showData(dayIndex, courseIndex) {
      if (
        this.weekCourse[dayIndex] &&
        this.weekCourse[dayIndex].courses[courseIndex] &&
        this.weekCourse[dayIndex].courses[courseIndex].index === courseIndex + 1
      ) {
        // this.getRandomColor();
        return this.weekCourse[dayIndex].courses[courseIndex];
      }
      return false;
    },
